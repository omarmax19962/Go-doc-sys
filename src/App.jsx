...
/* Sick-leave certificate — opens a print-ready window (Print → Save as PDF).
   Issued under a fixed signing doctor with a GoDoc stamp + signature block. */
const GODOC_SICK_LEAVE_DOCTOR = "Dr. Omar Youssef";
const printSickLeave = (patient, opts={}) => {
  ...
  <style>
    *{box-sizing:border-box;} body{font-family:-apple-system,Segoe UI,Roboto,sans-serif;color:#1E2A3A;margin:0;padding:36px;}
    .wrap{max-width:760px;margin:0 auto;position:relative;}
    .head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #7DD8DF;padding-bottom:16px;margin-bottom:8px;}
    .brand{font-size:28px;font-weight:800;letter-spacing:-.5px;} .brand span{color:#3FA796;}
    .muted{color:#8794A1;font-size:12px;} h1{font-size:15px;text-transform:uppercase;letter-spacing:1px;margin:0 0 4px;}
    .meta{text-align:right;font-size:12px;color:#5b6675;line-height:1.7;}
    .title{text-align:center;font-size:21px;font-weight:800;letter-spacing:.5px;text-transform:uppercase;margin:26px 0 4px;}
    .titlerule{width:90px;height:3px;background:#3FA796;margin:0 auto 26px;border-radius:3px;}
    .body{font-size:15px;line-height:2.0;}
    .body b{color:#1E2A3A;}
    .uline{border-bottom:1.5px solid #1E2A3A;padding:0 6px;font-weight:700;}
    .box{background:#F3F6F7;border-radius:12px;padding:14px 18px;margin:22px 0;font-size:14px;line-height:1.7;}
    .box b{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:#8794A1;margin-bottom:4px;}
    .sign{display:flex;justify-content:space-between;align-items:flex-end;margin-top:54px;}
    .sigblock{text-align:center;min-width:240px;}
    .signame{font-family:'Snell Roundhand','Brush Script MT',cursive;font-size:34px;color:#1E2A3A;line-height:1;margin-bottom:2px;}
    .sigrule{border-top:1.5px solid #1E2A3A;padding-top:6px;font-size:13px;font-weight:700;}
    .sigsub{font-size:11px;color:#8794A1;margin-top:2px;}
    .stamp{width:118px;height:118px;border:3px double #3FA796;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#3FA796;transform:rotate(-12deg);opacity:.92;}
    .stamp .s1{font-size:21px;font-weight:800;letter-spacing:1px;} .stamp .s1 span{color:#2E6E73;}
    ...
    <div class="body">
      This is to certify that <span class="uline">${esc(patient.name||"—")}</span>${patient.age!=null&&patient.age!==""?`, ${esc(String(patient.age))} years old`:""}${patient.phone?` (Tel: ${esc(patient.phone)})`:""}
      has been medically examined and is found to be unfit for work / study, and is therefore advised to take medical rest for
  ...
  if(!w){ alert("Please allow pop-ups to generate the sick leave note."); return; }
...
function Dashboard({patients,visits,notes,finances,credits=[],doctors,tasks,nameOf,goTab,openPatient,updateVisitStatus}){
  ...
  // Sessions a doctor added that still need the admin's approval.
  const toApprove=visits.filter(v=>dOf(v)&&v.createdBy==="doctor"&&v.approved===false&&v.status!=="cancelled"&&v.status!=="rescheduled");
  ...
  const items=[
    ...
    {k:"approve",Icon:Check,color:C.green,label:"Sessions to approve",n:toApprove.length,sub:"Doctor-added · awaiting your OK",go:()=>goTab("calendar")},
...
function EventPopover({ev,nameOf,onClose,updateVisitStatus,sendReminder,resolveReschedule,rescheduleVisit,deleteVisit,approveVisit,onActionReschedule}){
  const v=ev.raw;if(!v)return null;
  const doctorAdded=v.createdBy==="doctor";const needsApproval=doctorAdded&&v.approved===false;
  const[edate,setEdate]=useState(v.date||"");const[etime,setEtime]=useState(v.time&&v.time!=="—"?v.time:"");const[saved,setSaved]=useState(false);const[confirmDel,setConfirmDel]=useState(false);
  ...
  return(<div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(30,42,58,0.5)"}} onClick={onClose}>
    <div className="w-full max-w-[360px] rounded-2xl overflow-hidden" style={{background:"#fff"}} onClick={e=>e.stopPropagation()}>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-1.5 text-[11px] flex-wrap" style={{color:C.ink2}}><span className="font-bold uppercase text-[10px]" style={{color:C.grey}}>Source</span> · {doctorAdded?`Added by ${v.doctorName||"doctor"}`:"Added by Admin"}
          {doctorAdded&&(needsApproval
            ?<span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{background:C.amber+"22",color:"#9a6a00"}}>Pending approval</span>
            :<span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{background:C.green+"18",color:C.green}}>Approved ✓</span>)}</div>
        {needsApproval&&approveVisit&&<button onClick={()=>{approveVisit(v.id);onClose();}} className="w-full py-2.5 rounded-lg text-[12px] font-bold flex items-center justify-center gap-1.5" style={{background:C.green,color:"#fff"}}><Check size={14}/>Approve this session</button>}
        ...
        {deleteVisit&&!v.soapFiled&&v.status!=="completed"&&<div className="pt-1">
          {confirmDel
            ?<div className="flex items-center gap-2 text-[12px]"><span style={{color:C.grey}}>Delete this session?</span><button onClick={()=>{deleteVisit(v.id,"admin");onClose();}} className="font-bold px-3 py-1.5 rounded-lg text-white" style={{background:C.red}}>Delete</button><button onClick={()=>setConfirmDel(false)} className="font-semibold" style={{color:C.grey}}>Cancel</button></div>
            :<button onClick={()=>setConfirmDel(true)} className="flex items-center gap-1.5 text-[11.5px] font-bold" style={{color:C.red}}><Trash2 size={13}/>Delete session</button>}
        </div>}
      </div>
    </div>
  </div>);
...
function AdminCalendar({visits,patients,doctors,notes,nameOf,updateVisitStatus,sendReminder,resolveReschedule,rescheduleVisit,deleteVisit,approveVisit,bookSession}){
  ...
  const pendingApproval=visits.filter(v=>v.createdBy==="doctor"&&v.approved===false&&v.status!=="cancelled"&&v.status!=="rescheduled");
  const attention=[
    ...pendingApproval.map(v=>({v,tag:"Doctor-added · approve",c:C.green})),
  ...
  const events=useMemo(()=>visits
    ...
    .map(v=>({id:v.id,date:v.date,start:parseHM(v.time),durationMin:v.durationMin||45,
      title:nameOf(v.patientId),subtitle:`${v.doctorName} · ${(v.createdBy==="doctor"&&v.approved===false)?"⏳ Needs approval":(VISIT_STATUS_LABEL[v.status]||v.status)}`,
  ...
  return(<div>
    ...
    {attention.length>0&&<div className="rounded-2xl p-4 mb-4" style={{background:"#FFF8EC",border:`1px solid ${C.amber}55`}}>
      ...
      <div className="space-y-1.5">
        {attention.map(({v,tag,c,note,range},i)=>(<div key={v.id+tag+i} className="flex items-center justify-between bg-white rounded-xl px-3 py-2" style={{border:`1px solid ${C.line}`}}>
          ...
          <div className="flex items-center gap-1.5">
            {tag==="Doctor-added · approve"&&approveVisit&&<button onClick={()=>approveVisit(v.id)} className="text-[11px] font-semibold px-2.5 py-1 rounded-lg flex items-center gap-1" style={{background:C.green,color:"#fff"}}><Check size={12}/>Approve</button>}
    ...
    {popover&&<EventPopover ev={popover} nameOf={nameOf} onClose={()=>setPopover(null)} updateVisitStatus={updateVisitStatus} sendReminder={sendReminder} resolveReschedule={resolveReschedule} rescheduleVisit={rescheduleVisit} deleteVisit={deleteVisit} approveVisit={approveVisit} onActionReschedule={setReschedAction}/>}
...
/* ---------- SessionActions — inline edit-date + delete for a single session.
   Used by both doctor session cards and admin views. Stops click-propagation so
   it can live inside a clickable card. ---------- */
function SessionActions({v,rescheduleVisit,deleteVisit,by="admin"}){
  const[edit,setEdit]=useState(false);const[confirm,setConfirm]=useState(false);
  const[d,setD]=useState(v.date||"");const[t,setT]=useState(v.time&&v.time!=="—"?v.time:"");
  const stop=e=>e.stopPropagation();
  const save=async()=>{await rescheduleVisit(v.id,{date:d||null,time:t||null});setEdit(false);};
  return(<div className="mt-2 pt-2" style={{borderTop:`1px solid ${C.line}`}} onClick={stop}>
    {edit?(
      <div className="flex items-center gap-1.5 flex-wrap">
        <input type="date" value={d} onChange={e=>setD(e.target.value)} className="px-2 py-1.5 rounded-lg text-[12px] bg-white outline-none" style={{border:`1px solid ${C.line}`,color:d?C.ink:C.grey}}/>
        <input type="time" value={t} onChange={e=>setT(e.target.value)} step="900" className="px-2 py-1.5 rounded-lg text-[12px] bg-white outline-none" style={{border:`1px solid ${C.line}`,color:t?C.ink:C.grey}}/>
        <button onClick={save} className="text-[12px] font-bold px-2.5 py-1.5 rounded-lg text-white" style={{background:C.ink}}>Save</button>
        <button onClick={()=>{setEdit(false);setD(v.date||"");setT(v.time&&v.time!=="—"?v.time:"");}} className="text-[12px] font-semibold px-2 py-1.5" style={{color:C.grey}}>Cancel</button>
      </div>
    ):confirm?(
      <div className="flex items-center gap-2 text-[12px]"><span style={{color:C.grey}}>Delete this session?</span>
        <button onClick={()=>deleteVisit(v.id,by)} className="font-bold px-2.5 py-1 rounded-lg text-white" style={{background:C.red}}>Delete</button>
        <button onClick={()=>setConfirm(false)} className="font-semibold" style={{color:C.grey}}>Cancel</button></div>
    ):(
      <div className="flex items-center gap-4">
        <button onClick={()=>setEdit(true)} className="text-[11.5px] font-semibold flex items-center gap-1" style={{color:C.ink2}}><Pencil size={12}/>Edit date</button>
        <button onClick={()=>setConfirm(true)} className="text-[11.5px] font-semibold flex items-center gap-1" style={{color:C.red}}><Trash2 size={12}/>Delete</button>
      </div>
    )}
  </div>);
}

function UpcomingSessionRow({v,rescheduleVisit,deleteVisit,onAddDetails}){
  const[d,setD]=useState(v.date||"");const[t,setT]=useState(v.time&&v.time!=="—"?v.time:"");const[saved,setSaved]=useState(false);const[confirm,setConfirm]=useState(false);
  ...
  return(<div className="px-5 py-3 flex items-center gap-2 flex-wrap" style={{borderTop:`1px solid ${C.line}`}}>
    ...
    <div className="ml-auto flex items-center gap-2">
      ...
      {deleteVisit&&(confirm
        ?<span className="flex items-center gap-1.5 text-[11px]"><button onClick={()=>deleteVisit(v.id,"admin")} className="font-bold px-2.5 py-1.5 rounded-lg text-white" style={{background:C.red}}>Delete</button><button onClick={()=>setConfirm(false)} className="font-semibold" style={{color:C.grey}}>Cancel</button></span>
        :<button onClick={()=>setConfirm(true)} title="Delete session" className="p-1.5 rounded-lg" style={{border:`1px solid ${C.line}`}}><Trash2 size={13} color={C.grey}/></button>)}
    </div>
  </div>);
}
...
function SickLeaveModal({patient,onClose}){
  ...
  return(<div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{background:"rgba(30,42,58,0.55)"}} onClick={onClose}>
    <div className="w-full max-w-[460px] rounded-3xl overflow-hidden" style={{background:C.bg}} onClick={e=>e.stopPropagation()}>
      <div className="px-5 py-4 flex items-center justify-between" style={{background:"#fff",borderBottom:`1px solid ${C.line}`}}>
        <div className="flex items-center gap-2"><FileText size={18} color={C.ink}/><h2 className="text-[17px] font-bold" style={{fontFamily:HEAD}}>Sick leave note</h2></div>
        <button onClick={onClose}><X size={20} color={C.grey}/></button>
      </div>
      <div className="p-5 space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={generate} className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-white" style={{background:C.ink}}><Printer size={16}/>Generate &amp; print</button>
        </div>
      </div>
    </div>
  </div>);
      ...
      {mode==="profile"&&sub==="sessions"&&<div className="p-6">
        {role==="admin"&&rescheduleVisit&&(()=>{const up=upcoming.filter(v=>v.status!=="cancelled"&&v.status!=="rescheduled").slice().sort((a,b)=>(a.date||"9999").localeCompare(b.date||"9999"));const undated=up.filter(v=>!v.date).length;return up.length>0?(
          <div className="bg-white rounded-2xl overflow-hidden mb-4" style={{border:`1px solid ${C.line}`}}>
            ...
            {up.map(v=><UpcomingSessionRow key={v.id} v={v} rescheduleVisit={rescheduleVisit} deleteVisit={deleteVisit} onAddDetails={role==="admin"&&submitNote?setSoapVisit:undefined}/>)}
          </div>):null;})()}
...
function Doctor({patients,visits,notes,me,doctors,exerciseLib,modalityLib,packages,submitNote,assignSessionDate,bookSession,rescheduleVisit,deleteVisit,updateDoctorSlots,updateDoctorZones,updatePatientFiles,notifs,markRead}){
  ...
  // All of the doctor's open sessions — package billing is admin-only, so package
  // sessions just show here as ordinary sessions the doctor performs & documents.
  const mine=visits.filter(v=>v.doctorName===me&&v.status!=="completed"&&v.status!=="cancelled"&&v.status!=="rescheduled");
    ...
    {!active?<>
      ...
      {tab==="visits"&&<div className="p-4">{mine.length===0&&<p className="text-[13px] text-center mt-6" style={{color:C.grey}}>No upcoming visits — tap “Log a session” to log one independently.</p>}
        {mine.map(v=>{const p=pOf(v.patientId);return(<div key={v.id} className="bg-white rounded-2xl p-4 mb-3" style={{border:`1px solid ${C.line}`}}>
          ...
          <div className="flex items-center gap-2"><Clock size={14} color={C.teal}/><span className="font-bold">{v.date?(v.date+(v.time&&v.time!=="—"?" · "+to12(v.time):"")):"No date set"}</span><span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{background:v.type==="Assessment"?C.teal+"33":"#F0F0EE",color:C.ink2}}>{v.type}</span></div>
          ...
          <SessionActions v={v} rescheduleVisit={rescheduleVisit} deleteVisit={deleteVisit} by="doctor"/></div>);})}
      </div>}
      ...
      {tab==="calendar"&&<div className="p-3">
        ...
        <TimeGrid defaultView="day" compact
          events={visits.filter(v=>v.doctorName===me&&v.status!=="cancelled"&&v.status!=="rescheduled").map(v=>({id:v.id,date:v.date,start:parseHM(v.time),durationMin:v.durationMin||45,title:pOf(v.patientId)?.name||"—",subtitle:(v.createdBy==="doctor"&&v.approved===false)?"⏳ Pending admin approval":(VISIT_TYPE_LABEL[v.type]||v.type),color:C.teal,faded:v.status==="completed"||v.status==="no_show",noSoap:!v.soapFiled&&!!v.date&&v.date<=ymd(new Date())&&v.status!=="no_show",raw:v}))}
          onSlotClick={(date,time)=>setBooking({date,time})}
      ...
      <div className="bg-white rounded-2xl p-4" style={{border:`1px solid ${dxPlan?C.tealSoft:C.line}`}}>
        ...
        {dxPlan&&<>
          <div className="flex items-center justify-between mb-1.5">
            <div className="text-[11px] font-bold flex items-center gap-1.5" style={{color:"#2E6E73"}}><BookOpen size={12}/>Recommended · {dxPlan.label}</div>
          ...
          <p className="text-[10px] mt-2 mb-3" style={{color:C.grey}}>{dxPlan.src} · adjust dosage to the patient.</p>
          <div className="text-[10px] font-bold uppercase tracking-wider mb-1.5 pt-1" style={{color:C.grey,borderTop:`1px solid ${C.line}`}}>Other exercises</div>
        </>}
